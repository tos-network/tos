...
use tos_common::{
    ...
    prompt::{
        ...
        default_logs_datetime_format, Color, LogLevel, ModuleConfig, Prompt, PromptError,
        ShareablePrompt,
    },
    rpc::server::WebSocketServerHandler,
    serializer::Serializer,
    transaction::{verify::NoZKPCache, Transaction},
    utils::{format_difficulty, format_hashrate, format_tos},
};
...
async fn main() -> Result<()> {
    ...
    if let Some(path) = config.config_file.as_ref() {
        ...
        println!(
            "Using config from '{}', any launch option set will be discarded",
            path
        );
...
async fn run_prompt<S: Storage>(
    ...
    command_manager.add_command(Command::with_arguments(
        ...
        vec![Arg::new(
            "history",
            ArgType::Number,
            "Number of historical blocks to show",
        )],
    ...
    command_manager.add_command(Command::with_required_arguments(
        ...
        vec![Arg::new("hash", ArgType::Hash, "Block or transaction hash")],
    ...
    command_manager.add_command(Command::new(
        "top_block",
        "Print top block",
        CommandHandler::Async(async_handler!(top_block::<S>)),
    ))?;
    command_manager.add_command(Command::with_required_arguments(
        "pop_blocks",
    ...
    command_manager.add_command(Command::with_arguments(
        ...
        vec![Arg::new(
            "broadcast",
            ArgType::Bool,
            "Broadcast transaction to network (true/false)",
        )],
    ...
    command_manager.add_command(Command::with_required_arguments(
        ...
        vec![Arg::new(
            "topoheight",
            ArgType::Number,
            "Topological height to prune to",
        )],
    ...
    command_manager.add_command(Command::new(
        ...
        CommandHandler::Async(async_handler!(status::<S>)),
    ))?;
    command_manager.add_command(Command::with_optional_arguments(
        "blacklist",
        "View blacklist or add a peer ip in it",
        vec![Arg::new("ip", ArgType::String, "IP address to blacklist")],
        CommandHandler::Async(async_handler!(blacklist::<S>)),
    ))?;
    command_manager.add_command(Command::with_optional_arguments(
        "whitelist",
        "View whitelist or add a peer ip in it",
        vec![Arg::new("ip", ArgType::String, "IP address to whitelist")],
    ...
    command_manager.add_command(Command::with_required_arguments(
        ...
        vec![
            Arg::new("address", ArgType::String, "IP address to ban"),
            Arg::new("duration", ArgType::String, "Ban duration (e.g., 1h, 30m)"),
        ],
    ...
    command_manager.add_command(Command::new(
        ...
        CommandHandler::Async(async_handler!(clear_caches::<S>)),
    ))?;
    command_manager.add_command(Command::new(
        "clear_rpc_connections",
        "Clear all WS connections from RPC",
        CommandHandler::Async(async_handler!(clear_rpc_connections::<S>)),
    ))?;
    command_manager.add_command(Command::new(
        "clear_getwork_connections",
        "Clear all WS connections from GetWork",
        CommandHandler::Async(async_handler!(clear_miners_connections::<S>)),
    ))?;
    command_manager.add_command(Command::new(
        "clear_p2p_connections",
        "Clear all P2P connections",
        CommandHandler::Async(async_handler!(clear_p2p_connections::<S>)),
    ))?;
    command_manager.add_command(Command::new(
        "clear_p2p_peerlist",
        "Clear P2P peerlist",
        CommandHandler::Async(async_handler!(clear_p2p_peerlist::<S>)),
    ))?;
    command_manager.add_command(Command::with_optional_arguments(
        "difficulty_dataset",
    ...
    command_manager.add_command(Command::new(
        "list_unexecuted_transactions",
        "List all unexecuted transactions",
        CommandHandler::Async(async_handler!(list_unexecuted_transactions::<S>)),
    ))?;
    command_manager.add_command(Command::new(
        "swap_blocks_executions_positions",
        "Swap the position of two blocks executions",
        CommandHandler::Async(async_handler!(swap_blocks_executions_positions::<S>)),
    ))?;
    command_manager.add_command(Command::new(
        "print_balance",
        "Print the encrypted balance at a specific topoheight",
        CommandHandler::Async(async_handler!(print_balance::<S>)),
    ))?;
    command_manager.add_command(Command::new(
        "estimate_db_size",
        "Estimate the database total size",
        CommandHandler::Async(async_handler!(estimate_db_size::<S>)),
    ))?;
    command_manager.add_command(Command::new(
        "list_orphaned_blocks",
        "List all orphaned blocks we currently hold",
        CommandHandler::Async(async_handler!(list_orphaned_blocks::<S>)),
    ))?;
    command_manager.add_command(Command::new(
        "show_json_config",
        "Show the current config in JSON",
        CommandHandler::Async(async_handler!(show_json_config::<S>)),
    ))?;
    command_manager.add_command(Command::with_optional_arguments(
        "export_json_config",
        ...
        vec![Arg::new(
            "filename",
            ArgType::String,
            "Output filename for config export",
        )],
        CommandHandler::Async(async_handler!(export_json_config::<S>)),
    ))?;
    command_manager.add_command(Command::new(
        "broadcast_txs",
        "Broadcast all TXs in mempool if not done",
        CommandHandler::Async(async_handler!(broadcast_txs::<S>)),
    ))?;
    command_manager.add_command(Command::new(
        "snapshot_mode",
...
fn build_prompt_message(
    ...
    let network_hashrate_str = format!(
        ...
        prompt.colorize_string(
            Color::Green,
            &format!("{}", format_hashrate(network_hashrate))
        ),
    );
...
async fn verify_chain<S: Storage>(
    ...
    for topo in pruned_topoheight..=topoheight {
        ...
        let block_reward = if pruned_topoheight == 0 || topo - pruned_topoheight > STABLE_LIMIT {
            let block_reward = blockchain
                .get_block_reward(&*storage, &hash_at_topo, expected_supply, topo)
                .await
                .context("Error while calculating block reward")?;
            let expected_block_reward = storage
            ...
            storage
                .get_block_reward_at_topo_height(topo)
                .context("Error while retrieving block reward for pruned topo")?
        };

        let supply = storage
            .get_supply_at_topo_height(topo)
            .await
        ...
        if supply != expected_supply {
            manager.error(format!(
                "Error for block {} at topoheight {}, expected supply {} found {}",
                hash_at_topo,
                topo,
                format_tos(expected_supply),
                format_tos(supply)
            ));
            return Ok(());
        }
        ...
        for tx in block.get_transactions() {
                ...
                .context("Error while checking if tx is executed in block")?
            {
                ...
                if !storage
                    .has_nonce_at_exact_topoheight(transaction.get_source(), topo)
                    .await
                    .context("Error while checking the tx source nonce version")?
                {
                    manager.error(format!(
                        "No nonce version found for source {} at topoheight {}",
                        transaction
                            .get_source()
                            .as_address(blockchain.get_network().is_mainnet()),
                        topo
                ...
                for asset in transaction.get_assets() {
                    if !storage
                        .has_balance_at_exact_topoheight(transaction.get_source(), asset, topo)
                        .await
                        .context("Error while checking the tx source balance version")?
                    {
                        manager.error(format!(
                            "No balance version found for source {} at topoheight {}",
                            transaction
                                .get_source()
                                .as_address(blockchain.get_network().is_mainnet()),
                            topo
        ...
        let burned_supply = storage
            .get_burned_supply_at_topo_height(topo)
            .await
            .with_context(|| {
                format!("Error while retrieving burned supply at topoheight {topo}")
            })?;
        ...
        if burned_supply != expected_burned_supply {
            manager.error(format!(
                "Error for block {} at topoheight {}, expected burned supply {} found {}",
                hash_at_topo,
                topo,
                format_tos(expected_burned_supply),
                format_tos(burned_supply)
            ));
            return Ok(());
        }
    }
...
async fn list_unexecuted_transactions<S: Storage>(
    manager: &CommandManager,
    _: ArgumentManager,
) -> Result<(), CommandError> {
    let context = manager.get_context().lock()?;
...
async fn swap_blocks_executions_positions<S: Storage>(
    manager: &CommandManager,
    _: ArgumentManager,
) -> Result<(), CommandError> {
    let context = manager.get_context().lock()?;
    ...
    storage
        .swap_blocks_executions_positions(&left, &right)
        .await
...
async fn print_balance<S: Storage>(
    manager: &CommandManager,
    _: ArgumentManager,
) -> Result<(), CommandError> {
    let context = manager.get_context().lock()?;
    ...
    let balance = storage
        .get_balance_at_exact_topoheight(&address.to_public_key(), &asset, topoheight)
        .await
...
async fn estimate_db_size<S: Storage>(
    manager: &CommandManager,
    _: ArgumentManager,
) -> Result<(), CommandError> {
    let context = manager.get_context().lock()?;
    ...
    let size = storage
        .estimate_size()
        .await
        .context("Error while estimating size")?;
    manager.message(format!("Estimated size: {}", human_bytes(size as f64)));

    Ok(())
}

async fn list_orphaned_blocks<S: Storage>(
    manager: &CommandManager,
    _: ArgumentManager,
) -> Result<(), CommandError> {
    let context = manager.get_context().lock()?;
...
async fn export_json_config<S: Storage>(
    manager: &CommandManager,
    mut args: ArgumentManager,
) -> Result<(), CommandError> {
    let context = manager.get_context().lock()?;
    ...
    let path = if args.has_argument("filename") {
        ...
        manager
            .get_prompt()
            .read_input("Path to export the config: ", false)
            .await
    ...
    file.write_all(json.as_bytes())
        .context("Error while writing config file")?;
    file.flush().context("Error while flushing config file")?;
...
async fn broadcast_txs<S: Storage>(
    manager: &CommandManager,
    _: ArgumentManager,
) -> Result<(), CommandError> {
    let context = manager.get_context().lock()?;
    ...
    let txs = {
        ...
        mempool.get_txs().keys().cloned().collect::<Vec<_>>()
    };
    ...
    match blockchain.get_p2p().read().await.as_ref() {
        Some(p2p) => {
            ...
            if let Some(peer) = peer {
                peer.signal_exit()
                    .await
                    .context("Error while closing peer connection")?;
...
async fn temp_ban_address<S: Storage>(
    manager: &CommandManager,
    mut args: ArgumentManager,
) -> Result<(), CommandError> {
    let context = manager.get_context().lock()?;
    ...
    match blockchain.get_p2p().read().await.as_ref() {
        Some(p2p) => {
            let addr: IpAddr = args
                ...
                .parse()
                .context("Error while parsing socket address")?;
            let duration: HumanDuration = args
                .get_value("duration")?
                .to_string_value()?
                .parse()
                .context("Error while parsing duration")?;
            let peer_list = p2p.get_peer_list();

            peer_list
                .temp_ban_address(&addr, duration.as_secs(), true)
                .await
                .context("Error while banning address")?;
...
async fn list_miners<S: Storage>(
    manager: &CommandManager,
...
async fn list_assets<S: Storage>(
    manager: &CommandManager,
    mut arguments: ArgumentManager,
) -> Result<(), CommandError> {
    let page = if arguments.has_argument("page") {
...
async fn show_stored_peerlist<S: Storage>(
    manager: &CommandManager,
    mut arguments: ArgumentManager,
) -> Result<(), CommandError> {
    let page = if arguments.has_argument("page") {
    ...
    match blockchain.get_p2p().read().await.as_ref() {
        Some(p2p) => {
            let peer_list = p2p.get_peer_list();
            let peerlist: Vec<_> = peer_list
                .get_peerlist_entries()
                .collect::<Result<Vec<_>, _>>()
                .context("Error while retrieving stored peerlist")?;
            ...
            manager.message(format!(
                "Stored peerlist (total {}) page {}/{}:",
                peerlist.len(),
                page,
                max_pages
            ));
            for (ip, state) in peerlist
                .iter()
                .skip((page - 1) * ELEMENTS_PER_PAGE)
                .take(ELEMENTS_PER_PAGE)
            {
...
async fn show_balance<S: Storage>(
    manager: &CommandManager,
    mut arguments: ArgumentManager,
) -> Result<(), CommandError> {
    let prompt = manager.get_prompt();
    ...
    let mut history = if arguments.has_argument("history") {
        ...
        if value == 0 {
            return Err(CommandError::InvalidArgument(
                "history must be a positive number".into(),
            ));
        }
        value
    } else {
    ...
    if !storage
        ...
        .await
        .context("Error while checking if address has balance")?
    {
        manager.message("No balance found for address");
        return Ok(());
    }

    let (mut topo, mut version) = storage
    ...
    loop {
        ...
        if let Some(previous) = version.get_previous_topoheight() {
            version = storage
                ...
                .await
                .context("Error while retrieving history balance")?;
            topo = previous;
        } else {
            break;
        }
...
async fn clear_mempool<S: Storage>(
    manager: &CommandManager,
    _: ArgumentManager,
) -> Result<(), CommandError> {
    let context = manager.get_context().lock()?;
...
async fn snapshot_mode<S: Storage>(
    manager: &CommandManager,
    _: ArgumentManager,
) -> Result<(), CommandError> {
    let context = manager.get_context().lock()?;
...
async fn add_tx<S: Storage>(
    ...
    let tx =
        Transaction::from_hex(&hex).context("Error while decoding tx in hexadecimal format")?;
    let hash = tx.hash();
    ...
    blockchain
        ...
        .await
        .context("Error while adding TX to mempool")?;
...
async fn prune_chain<S: Storage>(
    ...
    manager.message(format!(
        "Chain has been pruned until topoheight {}",
        pruned_topoheight
    ));
    Ok(())
}

async fn status<S: Storage>(
    manager: &CommandManager,
    _: ArgumentManager,
) -> Result<(), CommandError> {
    let context = manager.get_context().lock()?;
    ...
    let top_block_hash = blockchain
        .get_top_block_hash_for_storage(&storage)
        .await
        .context("Error while retrieving top block hash")?;
    let avg_block_time = blockchain
    ...
    manager.message(format!(
        "Network Hashrate: {}",
        format_hashrate((difficulty / (block_time_target / MILLIS_PER_SECOND)).into())
    ));
    manager.message(format!("Top block hash: {}", top_block_hash));
    manager.message(format!(
        "Average Block Time: {:.2}s",
        avg_block_time as f64 / MILLIS_PER_SECOND as f64
    ));
    manager.message(format!(
        "Target Block Time: {:.2}s",
        block_time_target as f64 / MILLIS_PER_SECOND as f64
    ));
    manager.message(format!(
        "Emitted Supply: {} TOS",
        format_tos(emitted_supply)
    ));
    manager.message(format!("Burned Supply: {} TOS", format_tos(burned_supply)));
    manager.message(format!(
        "Circulating Supply: {} TOS",
        format_tos(emitted_supply - burned_supply)
    ));
    manager.message(format!(
        "Current Block Reward: {} TOS",
        format_tos(get_block_reward(emitted_supply, block_time_target))
    ));
    manager.message(format!(
        "Accounts/Transactions/Blocks/Assets/Contracts: {}/{}/{}/{}/{}",
    ...
    if let Some(pruned_topoheight) = pruned_topoheight {
        manager.message(format!(
            "Chain is pruned until topoheight {}",
            pruned_topoheight
        ));
    } else {
...
async fn clear_rpc_connections<S: Storage>(
    manager: &CommandManager,
    _: ArgumentManager,
) -> Result<(), CommandError> {
    let context = manager.get_context().lock()?;
...
async fn clear_miners_connections<S: Storage>(
    manager: &CommandManager,
    _: ArgumentManager,
) -> Result<(), CommandError> {
    let context = manager.get_context().lock()?;
    ...
    match blockchain
        ...
        .await
        .as_ref()
        .and_then(|rpc| rpc.getwork_server().as_ref())
    {
...
async fn clear_p2p_connections<S: Storage>(
    manager: &CommandManager,
    _: ArgumentManager,
) -> Result<(), CommandError> {
    let context = manager.get_context().lock()?;
...
async fn clear_p2p_peerlist<S: Storage>(
    manager: &CommandManager,
    _: ArgumentManager,
) -> Result<(), CommandError> {
    let context = manager.get_context().lock()?;
    ...
    match blockchain.get_p2p().read().await.as_ref() {
        Some(p2p) => {
            ...
            peerlist
                .clear_peerlist()
                .await
                .context("Error while clearing peerlist")?;
        ...
        Some(p2p) => {
            if arguments.has_argument("ip") {
                ...
                if peer_list
                    ...
                    .await
                    .context("Error while checking if peer is blacklisted")?
                {
                    peer_list
                        .set_graylist_for_peer(&address)
                        .await
                        .context("Error while setting graylist")?;
                    manager.message(format!("Peer {} is not blacklisted anymore", address));
                } else {
                    peer_list
                        .blacklist_address(&address)
                        .await
                        .context("Error while blacklisting peer")?;
                    manager.message(format!("Peer {} has been blacklisted", address));
                }
            } else {
                let peer_list = p2p.get_peer_list();
                let blacklist = peer_list
                    .get_blacklist()
                    .context("Error while retrieving blacklist")?;
        ...
        Some(p2p) => {
            if arguments.has_argument("ip") {
                ...
                if peer_list
                    ...
                    .await
                    .context("Error while checking if peer is whitelisted")?
                {
                    peer_list
                        .set_graylist_for_peer(&address)
                        .await
                    ...
                    peer_list
                        ...
                        .await
                        .context("Error while whitelisting peer")?;
                    manager.message(format!("Peer {} has been whitelisted", address));
                }
            } else {
                let peer_list = p2p.get_peer_list();
                let whitelist = peer_list
                    .get_whitelist()
                    .context("Error while retrieving whitelist")?;
...
async fn difficulty_dataset<S: Storage>(
    manager: &CommandManager,
    mut arguments: ArgumentManager,
) -> Result<(), CommandError> {
    ...
    for topoheight in 0..=blockchain.get_topo_height() {
        ...
        let (solve_time, difficulty, version, timestamp) = {
            ...
            let solve_time = if topoheight == 0 {
                ...
                let (_, tip_timestamp) = blockdag::find_newest_tip_by_timestamp::<S, _>(
                    &storage,
        ...
        file.write(
            format!(
                "{},{},{},{},{}\n",
                topoheight, solve_time, difficulty, hashrate, timestamp
            )
            .as_bytes(),
        )
...
async fn mine_block<S: Storage>(
    ...
    let prompt = manager.get_prompt();
    manager.message(format!(
        "Mining can take a while, are you sure you want to mine {} block(s)?",
        count
    ));
    if !prompt
        .ask_confirmation()
...
async fn add_peer<S: Storage>(
    ...
    match blockchain.get_p2p().read().await.as_ref() {
        Some(p2p) => {
            ...
            p2p.try_to_connect_to_peer(addr, false)
                .await
