use crate::{
    config::{
        get_genesis_block_hash, get_hex_genesis_block, get_stable_limit, DEV_FEES, DEV_PUBLIC_KEY,
        EMISSION_SPEED_FACTOR, GENESIS_BLOCK_DIFFICULTY, MAX_TIP_HEIGHT_DIFFERENCE,
        MILLIS_PER_SECOND, PRUNE_SAFETY_LIMIT, SIDE_BLOCK_REWARD_MAX_BLOCKS,
        SIDE_BLOCK_REWARD_MIN_PERCENT, SIDE_BLOCK_REWARD_PERCENT, STABLE_LIMIT,
    ...
    core::{
        ...
        storage::{CacheProvider, DagOrderProvider, DifficultyProvider, Storage},
...
use tos_common::{
    ...
    config::{
        COIN_DECIMALS, FEE_PER_KB, MAXIMUM_SUPPLY, MAX_BLOCK_SIZE, MAX_TRANSACTION_SIZE,
        TIPS_LIMIT, TOS_ASSET,
    },
...
use super::storage::{
    AccountProvider, BlocksAtHeightProvider, ClientProtocolProvider, PrunedTopoheightProvider,
};
...
/// Pre-verification cache for block validation.
/// This allows expensive operations (block hash, PoW hash) to be computed
/// in advance and cached for use during block addition.
pub enum PreVerifyBlock {
    /// Only the block hash is pre-computed
    Hash(Immutable<Hash>),
    /// Both block hash and PoW hash are pre-computed
    Partial {
        block_hash: Immutable<Hash>,
        pow_hash: Hash,
    },
    /// Nothing is pre-computed, will compute on demand
    None,
}

impl PreVerifyBlock {
    /// Get the block hash if pre-computed
    ...
    /// Get the PoW hash if pre-computed
    pub fn get_pow_hash(&self) -> Option<&Hash> {
        match self {
            Self::Partial { pow_hash, .. } => Some(pow_hash),
            _ => None,
        }
    }
}
...
pub struct Blockchain<S: Storage> {
    ...
    // NOTE: Cache fields (height, topoheight, stable_height, stable_topoheight, difficulty,
    // tip_base_cache, common_base_cache, tip_work_score_cache, full_order_cache)
    // are now stored in RocksStorage.cache for atomic snapshot rollback support.
    storage: RwLock<S>,
    ...
    // By default, set to N threads available (configurable via pre_verify_block_threads_count)
...
impl<S: Storage> Blockchain<S> {
    pub async fn new(
        ...
        // Initialize the storage cache with the loaded height and topoheight
        {
        ...
        if !on_disk {
            ...
            // Update the chain cache with computed values
            {
                let chain_cache = storage.chain_cache_mut().await?;
                    ...
                    match peer.parse() {
                        ...
                        Err(e) => {
                            match lookup_host(&peer).await {
                                ...
                                Err(e2) => {
                                    error!(
                                        "Error while parsing {} as exclusive node address: {}, {}",
                                        peer, e, e2
                                    );
                                }
                            };
    ...
    // Clear all caches (DAG operation caches and object caches)
    pub async fn clear_caches(&self) -> Result<(), BlockchainError> {
        debug!("Clearing caches...");
        let mut storage = self.storage.write().await;
        storage.chain_cache_mut().await?.clear_caches();
    ...
    pub async fn reload_from_disk_with_storage(
        ...
        let (stable_hash, stable_height) = self.find_common_base(&*storage, &tips, version).await?;
    ...
    pub async fn mine_block(&self, key: &PublicKey) -> Result<Block, BlockchainError> {
        ...
        while !self.is_simulator_enabled() && !check_difficulty(&hash, &difficulty)? {
            if self.get_height().await != current_height {
    ...
    pub async fn prune_until_topoheight_for_storage(
        ...
        let current_topoheight = self.get_topo_height_for_storage(&*storage).await;
            ...
            + PrunedTopoheightProvider,
    {
        while topoheight > 0 {
            ...
            if self
                .is_sync_block_at_height(provider, &block_hash, current_height, version)
                .await?
            {
    ...
    // NOTE: This is async as it requires storage lock
    pub async fn get_height(&self) -> u64 {
        self.storage.read().await.chain_cache().await.height
    }

    // returns the highest topological height
    // NOTE: This is async as it requires storage lock
    pub async fn get_topo_height(&self) -> TopoHeight {
        self.storage.read().await.chain_cache().await.topoheight
    }
    ...
    pub async fn get_stable_height(&self) -> u64 {
        self.storage.read().await.chain_cache().await.stable_height
    }
    ...
    // Helper methods for use when storage is already locked
    // These avoid re-acquiring locks in internal methods
    // NOTE: These are now async due to chain_cache() being async

    // Get height from already-locked storage
    #[inline]
    pub async fn get_height_for_storage(&self, storage: &S) -> u64 {
        storage.chain_cache().await.height
    }

    // Get topoheight from already-locked storage
    #[inline]
    pub async fn get_topo_height_for_storage(&self, storage: &S) -> TopoHeight {
        storage.chain_cache().await.topoheight
    }

    // Get stable height from already-locked storage
    #[inline]
    pub async fn get_stable_height_for_storage(&self, storage: &S) -> u64 {
        storage.chain_cache().await.stable_height
    }

    // Get stable topoheight from already-locked storage
    #[inline]
    pub async fn get_stable_topoheight_for_storage(&self, storage: &S) -> TopoHeight {
        storage.chain_cache().await.stable_topoheight
    }

    // Get difficulty from already-locked storage
    #[inline]
    pub async fn get_difficulty_for_storage(&self, storage: &S) -> Difficulty {
        storage.chain_cache().await.difficulty
    }
    ...
    pub async fn get_supply(&self) -> Result<u64, BlockchainError> {
        ...
        storage
            .get_supply_at_topo_height(self.get_topo_height_for_storage(&*storage).await)
            .await
    }
    ...
    pub async fn get_burned_supply(&self) -> Result<u64, BlockchainError> {
        ...
        storage
            .get_burned_supply_at_topo_height(self.get_topo_height_for_storage(&*storage).await)
            .await
    }
    ...
    pub async fn get_top_block_hash_for_storage(
        ...
        storage
            .get_hash_at_topo_height(self.get_topo_height().await)
            .await
    }
    ...
    >(
        ...
        let version = get_version_at_height(self.get_network(), current_height);
        self.is_sync_block_at_height(provider, hash, current_height, version)
            .await
    }
            ...
            + PrunedTopoheightProvider,
    {
        ...
        if block_height == 0 {
            ...
            trace!(
                "Block {} at height {} is a sync block because it can only be the genesis block",
                hash,
                block_height
            );
            return Ok(true);
        ...
        let sync_block_cumulative_difficulty = provider
        ...
        for pre_hash in pre_blocks {
            ...
            if provider.is_block_topological_ordered(&pre_hash).await? {
                let cumulative_difficulty = provider
                    .get_cumulative_difficulty_for_block_hash(&pre_hash)
                    .await?;
            ...
            + CacheProvider,
    {
        ...
        let mut cache = provider.chain_cache().await.tip_base_cache.lock().await;
        ...
        'main: while let Some(current_hash) = stack.pop_back() {
            ...
            for tip_hash in tips.iter() {
                ...
                if self
                    .is_sync_block_at_height(provider, &tip_hash, height, version)
                    .await?
                {
                    let block_height = provider.get_height_for_block_hash(&tip_hash).await?;
                    // save in cache
        ...
        I: IntoIterator<Item = &'a Hash> + Copy,
    {
        ...
        let mut cache = provider.chain_cache().await.common_base_cache.lock().await;
        ...
        for hash in tips.into_iter() {
            ...
            bases.push(
                self.find_tip_base(provider, hash, best_height, pruned_topoheight, version)
                    .await?,
            );
        }
    ...
    async fn verify_non_reachability<P: DifficultyProvider>(
        ...
        for hash in tips {
            let set = self
                .build_reachability(provider, hash.clone(), block_version)
                .await?;
            reach.push(set);
        }
        ...
        P: DifficultyProvider + DagOrderProvider,
    {
        ...
        if lowest_ordered_height <= chain_height
            && chain_height - lowest_ordered_height >= get_stable_limit(version)
        {
            return Ok(false);
        }
    ...
    async fn find_tip_work_score_internal<'a, P>(
        &self,
        provider: &P,
        map: &mut HashMap<Hash, CumulativeDifficulty>,
        hash: &'a Hash,
        ...
        while let Some(current_hash) = stack.pop_back() {
            ...
            for tip_hash in tips.iter() {
                if !map.contains_key(tip_hash) {
                    ...
                    if !is_ordered
                        || (is_ordered
                            && provider.get_topo_height_for_hash(tip_hash).await?
                                >= base_topoheight)
                    {
                        stack.push_back(tip_hash.clone());
                    }
    ...
    {
        trace!(
            "find tip work score for {} at base {}",
            block_hash,
            base_block
        );
        ...
        for hash in block_tips {
            if !map.contains_key(hash) {
                ...
                if !is_ordered
                    || (is_ordered
                        && provider.get_topo_height_for_hash(hash).await? >= base_topoheight)
                {
                    self.find_tip_work_score_internal(provider, &mut map, hash, base_topoheight)
                        .await?;
                }
            }
        }
        ...
        if base_block != block_hash {
            map.insert(
                base_block.clone(),
                provider
                    .get_cumulative_difficulty_for_block_hash(base_block)
                    .await?,
            );
        }
        ...
        cache.put(
            (block_hash.clone(), base_block.clone(), base_block_height),
            (set.clone(), score),
        );
    ...
    async fn find_best_tip<'a, P: DifficultyProvider + DagOrderProvider + CacheProvider>(
        &self,
        provider: &P,
        tips: &'a HashSet<Hash>,
        base: &Hash,
        ...
        for hash in tips {
            ...
            let (_, cumulative_difficulty) = self
                .find_tip_work_score(provider, hash, block_tips.iter(), None, base, base_height)
                .await?;
        ...
        P: DifficultyProvider + DagOrderProvider + CacheProvider,
    {
        ...
        let mut cache = provider.chain_cache().await.full_order_cache.lock().await;
        ...
        'main: while let Some(current_hash) = stack.pop_back() {
            ...
            for tip_hash in block_tips.iter() {
                ...
                {
                    let diff = provider
                        .get_cumulative_difficulty_for_block_hash(tip_hash)
                        .await?;
        ...
        I::IntoIter: ExactSizeIterator,
    {
        ...
        let best_tip =
            blockdag::find_best_tip_by_cumulative_difficulty(provider, tips.clone().into_iter())
                .await?;
    ...
    pub async fn get_difficulty(&self) -> Difficulty {
        self.storage.read().await.chain_cache().await.difficulty
    }
    ...
    pub async fn add_tx_to_mempool_with_storage_and_hash(
        ...
        let hash = {
            ...
            let stable_topoheight = self.get_stable_topoheight().await;
            let current_topoheight = self.get_topo_height().await;
            ...
            let start = Instant::now();
            let version = get_version_at_height(self.get_network(), self.get_height().await);
            mempool
                .add_tx(
                    storage,
                    &self.environment,
    ...
    pub async fn get_block_header_template_for_storage(
        ...
        let current_height = self.get_height().await;
        if tips.len() > 1 {
            let best_tip = blockdag::find_best_tip_by_cumulative_difficulty(storage, tips.iter())
                .await?
                .clone();
            let block_height_by_tips =
                blockdag::calculate_height_at_tips(storage, tips.iter()).await?;
            ...
            for hash in tips {
                if best_tip != hash {
                    ...
                    if height_diff > MAX_TIP_HEIGHT_DIFFERENCE {
                        warn!(
                            "Tip {} has a height difference too big ({} > {})",
                            hash, height_diff, MAX_TIP_HEIGHT_DIFFERENCE
                        );
                        continue;
                    }
                }
        ...
        let block = BlockHeader::new(
            get_version_at_height(self.get_network(), height),
            height,
            timestamp,
            sorted_tips,
            extra_nonce,
    ...
    pub async fn get_block_template_for_storage(
        ...
        let stable_topoheight = self.get_stable_topoheight().await;
        let stable_height = self.get_stable_height().await;
        let topoheight = self.get_topo_height().await;
        ...
        if !tx_selector.is_empty() {
            ...
            let processed_txs = self
                .get_all_txs_until_height(
                    ...
                    false,
                    true, // Only process orphaned blocks
                )
                .await?;
    ...
    pub async fn build_block_from_header(
        ...
        for hash in header.get_txs_hashes() {
            ...
            let tx = if mempool.contains_tx(hash) {
                ...
                storage.get_transaction(hash).await?.into_arc()
            };
    ...
    /// Pre-verify a block by computing its block hash and PoW hash.
    /// This can be called in advance to avoid computing these expensive
    /// operations during the critical path of block addition.
    ///
    /// Returns a `PreVerifyBlock` that can be passed to `add_new_block`.
    pub async fn pre_verify_block(
        &self,
        block: &Block,
        block_hash: Option<Immutable<Hash>>,
    ) -> Result<PreVerifyBlock, BlockchainError> {
        // Acquire semaphore to limit concurrent pre-verifications
        // This prevents resource exhaustion from too many parallel PoW computations
        let _permit = self.pre_verify_block_semaphore.acquire().await?;

        // NOTE: Height will be verified at the add_new_block function
        let expected_version = get_version_at_height(self.get_network(), block.get_height());
        ...
        if self.skip_pow_verification() {
            ...
            Ok(PreVerifyBlock::Hash(
                block_hash.unwrap_or_else(|| Immutable::Owned(block.hash())),
            ))
        } else {
            ...
            tokio::task::spawn_blocking(move || {
                ...
                Ok::<_, BlockchainError>(PreVerifyBlock::Partial {
                    block_hash,
                    pow_hash,
                })
            })
            .await
            .map_err(|e| {
                error!("Task join error during pre-verification: {}", e);
    ...
    pub async fn add_new_block(
        ...
        // Either use pre-computed values or compute on demand
        let (block_hash, pre_computed_pow_hash) = match pre_verify {
            PreVerifyBlock::Hash(hash) => (hash, None),
            PreVerifyBlock::Partial {
                block_hash,
                pow_hash,
            } => (block_hash, Some(pow_hash)),
        ...
        let mut current_height = self.get_height_for_storage(&*storage).await;
        ...
        if block_height_by_tips != block.get_height() {
            debug!(
                "Invalid block height {}, expected {} for this block {}",
                block.get_height(),
        ...
        if tips_count > 0 {
            if log::log_enabled!(log::Level::Debug) {
                debug!(
                    "Height by tips: {}, stable height: {}",
                    block_height_by_tips, stable_height
                );
            }
        ...
        if !self
            .verify_non_reachability(&*storage, block.get_tips(), version)
            .await?
        {
            debug!(
        ...
        for hash in block.get_tips() {
            ...
            if block.get_timestamp() < previous_timestamp {
                debug!(
                    "Invalid block timestamp, parent ({}) is less than new block {}",
                    hash, block_hash
                );
            ...
            if !self
                .is_near_enough_from_main_chain(&*storage, hash, height, version)
                .await?
            {
                error!(
                    "{} with hash {} have deviated too much (current height: {}, block height: {})",
                    block, block_hash, current_height, block_height_by_tips
                );
                return Err(BlockchainError::BlockDeviation);
            }

            // Check that the tip is not too far in height from block height
            let tip_height = storage.get_height_for_block_hash(hash).await?;
            let height_diff = block_height_by_tips.saturating_sub(tip_height);
            if height_diff > MAX_TIP_HEIGHT_DIFFERENCE {
                debug!(
                    "Tip {} is too far back: height difference {} exceeds maximum {}",
                    hash, height_diff, MAX_TIP_HEIGHT_DIFFERENCE
                );
        ...
        if tips_count > 1 {
            let best_tip = blockdag::find_best_tip_by_cumulative_difficulty(
                &*storage,
                block.get_tips().iter(),
            )
            .await?;
        ...
        let pow_hash = if skip_pow {
            // Simulator is enabled, we don't need to compute the PoW hash
            Hash::zero()
        } else if let Some(cached_pow_hash) = pre_computed_pow_hash {
            // Use the pre-computed PoW hash from PreVerifyBlock::Partial
            cached_pow_hash
        } else {
            // Compute the PoW hash on demand
            let start = Instant::now();
            let algorithm = get_pow_algorithm_for_version(version);
        ...
        let (difficulty, p) = self
            .verify_proof_of_work(&*storage, &pow_hash, block.get_tips().iter())
            .await?;
        debug!("PoW is valid for difficulty {}", difficulty);

        let mut current_topoheight = self.get_topo_height_for_storage(&*storage).await;
        ...
        // If the TX is executed by another branch, we skip the verification because DAG will choose which branch will execute the TX
        {
            ...
            let parents_txs = if !block.get_txs_hashes().is_empty() {
                debug!(
                    "Loading all TXs until height {} for block {} (executed only: {}, orphaned only: {})",
                    stable_height, block_hash, !is_full_tx_handling_enabled, is_orphan_tx_grouping_enabled
                );
            ...
            for (tx, hash) in block.get_transactions().iter().zip(block.get_txs_hashes()) {
                ...
                if is_executed {
                    ...
                    let block_executor_height =
                        storage.get_height_for_block_hash(&block_executor).await?;
            ...
            if !txs_grouped.is_empty() {
                ...
                let start = Instant::now();
                let stable_topoheight = self.get_stable_topoheight_for_storage(&*storage).await;
        ...
        debug!(
            "Cumulative difficulty for block {}: {}",
            block_hash, cumulative_difficulty
        );
        ...
        let (base_hash, base_height) = self.find_common_base(&*storage, &tips, version).await?;
        debug!("New base hash: {}, height: {}", base_hash, base_height);
        let best_tip = self
            .find_best_tip(&*storage, &tips, &base_hash, base_height)
            .await?;
                ...
                for (tx, tx_hash) in block.get_transactions().iter().zip(block.get_txs_hashes()) {
                        ...
                        .is_tx_executed_in_a_block(tx_hash)?
                    {
                        ...
                        if expected_next_nonce != next_nonce {
                            warn!(
                                "TX {} has a nonce {}, but the next nonce is {}, forcing it...",
                                tx_hash, next_nonce, expected_next_nonce
                            );
                            chain_state
                                .as_mut()
                        ...
                        if orphaned_transactions.shift_remove(tx_hash) {
                            trace!(
                                "Transaction {} was marked as orphaned, but got executed again",
                                tx_hash
                            );
                        }
                                ...
                                if should_track_events.contains(&event) {
                                        ...
                                        chain_state.get_contract_outputs_for_tx(&tx_hash)
                                    {
                                        let contract_outputs = contract_outputs
                                            .into_iter()
                                            .map(|output| {
                                                RPCContractOutput::from_output(output, is_mainnet)
                                            })
                                            .collect::<Vec<_>>();
        ...
        for hash in tips {
            if self
                .is_near_enough_from_main_chain(&*storage, &hash, current_height, version)
                .await?
            {
        ...
        // update stable height and difficulty in cache
        {
            if should_track_events.contains(&NotifyEvent::StableHeightChanged) {
                ...
                let previous_stable_height = self.get_stable_height_for_storage(&*storage).await;
            ...
            if should_track_events.contains(&NotifyEvent::StableTopoHeightChanged) {
                ...
                let previous_stable_topoheight =
                    self.get_stable_topoheight_for_storage(&*storage).await;
    ...
    async fn get_all_txs_until_height<P>(
        ...
        txs_executed_only: bool,
        blocks_orphaned_only: bool,
    ) -> Result<IndexSet<Hash>, BlockchainError>
    where
