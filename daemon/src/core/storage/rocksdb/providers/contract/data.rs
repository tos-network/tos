...
impl ContractDataProvider for RocksStorage {
    // Set a contract data
    async fn set_last_contract_data_to(
        &mut self,
        contract: &Hash,
        key: &ValueCell,
    ...
    async fn get_last_topoheight_for_contract_data(
        &self,
        contract: &Hash,
        key: &ValueCell,
    ...
    async fn get_contract_data_at_exact_topoheight_for<'a>(
        &self,
        contract: &Hash,
        key: &ValueCell,
        topoheight: TopoHeight,
    ...
    async fn get_contract_data_at_maximum_topoheight_for<'a>(
        &self,
        contract: &Hash,
        key: &ValueCell,
        ...
        {
            let version = self
                .get_contract_data_at_exact_topoheight_for(contract, key, topo)
                .await?;
    ...
    async fn get_contract_data_topoheight_at_maximum_topoheight_for<'a>(
        &self,
        contract: &Hash,
        key: &ValueCell,
        maximum_topoheight: TopoHeight,
        ...
        let mut versioned_key =
            Self::get_versioned_contract_data_key(contract_id, &key, maximum_topoheight);
        let mut prev_topo: Option<TopoHeight> =
            self.load_optional_from_disk(Column::ContractsData, &versioned_key[8..])?;
    ...
    async fn has_contract_data_at_maximum_topoheight(
        &self,
        contract: &Hash,
        key: &ValueCell,
        maximum_topoheight: TopoHeight,
        ...
        while let Some(topo) = prev_topo {
            ...
            if topo <= maximum_topoheight {
                let has_data = self
                    .load_from_disk::<_, (Option<TopoHeight>, bool)>(
    ...
    async fn has_contract_data_at_exact_topoheight(
        &self,
        contract: &Hash,
        key: &ValueCell,
        topoheight: TopoHeight,
    ...
    async fn get_contract_data_entries_at_maximum_topoheight<'a>(
        &'a self,
        contract: &'a Hash,
        topoheight: TopoHeight,
    ) -> Result<
        impl Stream<Item = Result<(ValueCell, ValueCell), BlockchainError>> + Send + 'a,
        BlockchainError,
    > {
        let iterator = self.iter_keys::<u64>(
            Column::ContractsData,
...
impl RocksStorage {
    ...
    pub fn get_versioned_contract_data_key(
        contract: ContractId,
        key: &ValueCell,
        topoheight: TopoHeight,
    ) -> [u8; 24] {
