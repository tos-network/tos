use crate::core::{
    ...
    storage::{rocksdb::Column, RocksStorage, VersionedContractDataProvider},
};
...
impl VersionedContractDataProvider for RocksStorage {
    async fn delete_versioned_contract_data_at_topoheight(
        &mut self,
    ...
    async fn delete_versioned_contract_data_above_topoheight(
        &mut self,
        topoheight: TopoHeight,
    ) -> Result<(), BlockchainError> {
        trace!(
    ...
    async fn delete_versioned_contract_data_below_topoheight(
        &mut self,
        topoheight: TopoHeight,
        keep_last: bool,
    ) -> Result<(), BlockchainError> {
