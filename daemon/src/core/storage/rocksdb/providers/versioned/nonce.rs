...
use crate::core::{
    ...
    storage::{
        rocksdb::{Account, AccountId, Column, IteratorMode},
        NetworkProvider, RocksStorage, VersionedNonceProvider,
    },
};
...
impl VersionedNonceProvider for RocksStorage {
    ...
    async fn delete_versioned_nonces_at_topoheight(
        &mut self,
    ...
    async fn delete_versioned_nonces_above_topoheight(
        &mut self,
        topoheight: TopoHeight,
    ) -> Result<(), BlockchainError> {
    ...
    async fn delete_versioned_nonces_below_topoheight(
        &mut self,
        topoheight: TopoHeight,
        keep_last: bool,
    ) -> Result<(), BlockchainError> {
        ...
        if keep_last {
            for res in Self::iter_owned_internal::<(), Account>(
                &self.db,
                self.snapshot.as_ref(),
                IteratorMode::Start,
                Column::Account,
