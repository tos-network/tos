...
impl ContractDataProvider for SledStorage {
    ...
    async fn get_contract_data_at_maximum_topoheight_for<'a>(
            ...
            .await?
        {
            Some(topoheight) => {
                let contract = self
                    .get_contract_data_at_exact_topoheight_for(&contract, key, topoheight)
                    .await?;
    ...
    async fn get_contract_data_topoheight_at_maximum_topoheight_for<'a>(
        ...
        let topo = if self
            .has_contract_data_at_exact_topoheight(contract, key, maximum_topoheight)
            .await?
        {
            maximum_topoheight
        } else {
    ...
    async fn get_contract_data_entries_at_maximum_topoheight<'a>(
        ...
        BlockchainError,
    > {
        Ok(stream::iter(Self::scan_prefix(
        ...
        .map(move |res| async move {
            ...
            let value = self
                .get_contract_data_at_maximum_topoheight_for(contract, &k, topoheight)
                .await?;
...
impl SledStorage {
    ...
    pub fn get_versioned_contract_data_key_from_hash(
        &self,
        hash: &Hash,
        topoheight: TopoHeight,
    ) -> [u8; 40] {
