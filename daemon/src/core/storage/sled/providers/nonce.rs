use crate::core::{
    ...
    storage::{sled::ACCOUNTS_COUNT, AccountProvider, NetworkProvider, NonceProvider, SledStorage},
};
use async_trait::async_trait;
use log::trace;
use tos_common::{
    account::VersionedNonce, block::TopoHeight, crypto::PublicKey, serializer::Serializer,
};
...
impl NonceProvider for SledStorage {
    async fn set_last_nonce_to(
        ...
        let prev = Self::insert_into_disk(
            self.snapshot.as_mut(),
            &self.nonces,
            key.as_bytes(),
    ...
    async fn get_last_topoheight_for_nonce(
        &self,
        key: &PublicKey,
        ...
        self.load_from_disk(
            &self.nonces,
            key.as_bytes(),
            DiskContext::LastTopoheightForNonce,
        )
    }
    ...
    async fn has_nonce_at_exact_topoheight(
        &self,
        key: &PublicKey,
        topoheight: TopoHeight,
    ...
    async fn get_last_nonce(
        ...
        let topoheight =
            self.load_from_disk(&self.nonces, key.as_bytes(), DiskContext::LastNonce)?;
        Ok((
            topoheight,
            self.get_nonce_at_exact_topoheight(key, topoheight).await?,
        ))
    }

    async fn get_nonce_at_exact_topoheight(
        &self,
        key: &PublicKey,
        topoheight: TopoHeight,
        ...
        self.load_from_disk(
            &self.versioned_nonces,
            &key,
            DiskContext::NonceAtTopoHeight(topoheight),
        )
    }

    // topoheight is inclusive bounds
    async fn get_nonce_at_maximum_topoheight(
        &self,
        key: &PublicKey,
        topoheight: TopoHeight,
        ...
        while let Some(previous) = topo {
            ...
            topo = self.load_from_disk(
                &self.versioned_nonces,
                &Self::get_versioned_key(key.as_bytes(), previous),
                DiskContext::LastNonce,
            )?;
        }
