...
impl SledStorage {
    ...
    pub fn get_versioned_balance_key(
        &self,
        key: &PublicKey,
        asset: &Hash,
...
impl BalanceProvider for SledStorage {
    ...
    async fn has_balance_for(
        ...
        self.has_balance_internal(&self.get_balance_key_for(key, asset))
            .await
    }
    ...
    async fn get_last_topoheight_for_balance(
        ...
        if log::log_enabled!(log::Level::Trace) {
            trace!(
                "get last topoheight for balance {} for {}",
                asset,
                key.as_address(self.is_mainnet())
            );
        }
        ...
        self.get_cacheable_data(
            ...
            &None,
            &key,
            DiskContext::LastTopoHeightForBalance,
        )
        .await
    }
    ...
    fn set_last_topoheight_for_balance(
        ...
        if log::log_enabled!(log::Level::Trace) {
            trace!(
                "set last topoheight to {} for balance {} for {}",
                topoheight,
                asset,
                key.as_address(self.is_mainnet())
            );
    ...
    async fn has_balance_at_exact_topoheight(
        ...
        if log::log_enabled!(log::Level::Trace) {
            trace!(
                "has balance {} for {} at exact topoheight {}",
                asset,
                key.as_address(self.is_mainnet()),
                topoheight
            );
    ...
    async fn get_balance_at_exact_topoheight(
        ...
        if log::log_enabled!(log::Level::Trace) {
            trace!(
                "get balance {} for {} at exact topoheight {}",
                asset,
                key.as_address(self.is_mainnet()),
                topoheight
            );
        ...
        // check first that this address has balance, if no returns
        if !self
            .has_balance_at_exact_topoheight(key, asset, topoheight)
            .await?
        {
            if log::log_enabled!(log::Level::Trace) {
                trace!(
                    "No balance {} found for {} at exact topoheight {}",
                    asset,
                    key.as_address(self.is_mainnet()),
                    topoheight
                );
        ...
        self.get_cacheable_data(
            ...
            &None,
            &disk_key,
            DiskContext::BalanceAtTopoHeight(topoheight),
        )
        .await
        .map_err(|_| {
            BlockchainError::NoBalanceChanges(
                key.as_address(self.is_mainnet()),
                topoheight,
                asset.clone(),
            )
    ...
    async fn get_balance_at_maximum_topoheight(
        ...
        if log::log_enabled!(log::Level::Trace) {
            trace!(
                "get balance {} for {} at maximum topoheight {}",
                asset,
                key.as_address(self.is_mainnet()),
                topoheight
            );
        ...
        if !self.has_balance_for(key, asset).await? {
            if log::log_enabled!(log::Level::Trace) {
                trace!(
                    "No balance {} found for {} at maximum topoheight {}",
                    asset,
                    key.as_address(self.is_mainnet()),
                    topoheight
                );
        ...
        while let Some(topo) = previous_topoheight {
            if topo <= topoheight {
                let version = self
                    .get_balance_at_exact_topoheight(key, asset, topo)
                    .await?;
            ...
            previous_topoheight = self.load_from_disk(
                &self.versioned_balances,
                &self.get_versioned_balance_key(key, asset, topo),
    ...
    async fn get_new_versioned_balance(
        ...
        if log::log_enabled!(log::Level::Trace) {
            trace!(
                "get new versioned balance {} for {} at {}",
                asset,
                key.as_address(self.is_mainnet()),
                topoheight
            );
    ...
    async fn get_output_balance_at_maximum_topoheight(
        ...
        if log::log_enabled!(log::Level::Trace) {
            trace!(
                "get output balance {} for {} at maximum topoheight {}",
                asset,
                key.as_address(self.is_mainnet()),
                topoheight
            );
        ...
        while let Some(topo) = next {
            ...
            let (prev_topo, balance_type): (Option<u64>, BalanceType) = self.load_from_disk(
                &self.versioned_balances,
                &self.get_versioned_balance_key(key, asset, topo),
                DiskContext::BalanceAtTopoHeight(topo),
            )?;
    ...
    async fn get_output_balance_in_range(
        ...
        if log::log_enabled!(log::Level::Trace) {
            trace!(
                "get output balance {} for {} in range {} - {}",
                asset,
                key.as_address(self.is_mainnet()),
                min_topoheight,
                max_topoheight
        ...
        let topo = if self
            .has_balance_at_exact_topoheight(key, asset, max_topoheight)
            .await?
        {
            max_topoheight
        } else {
    ...
    async fn set_last_balance_to(
        ...
        if log::log_enabled!(log::Level::Trace) {
            trace!(
                "set balance {} for {} to topoheight {}",
                asset,
                key.as_address(self.is_mainnet()),
                topoheight
            );
        }
        self.set_balance_at_topoheight(asset, topoheight, key, &version)
            .await?;
    ...
    async fn get_last_balance(
        ...
        let topoheight = self
            .get_cacheable_data(
                &self.balances,
                &None,
                &self.get_balance_key_for(key, asset),
                DiskContext::LastBalance,
            )
            .await?;
        let version = self
            .get_balance_at_exact_topoheight(key, asset, topoheight)
            .await?;
    ...
    async fn get_account_summary_for(
        ...
        if log::log_enabled!(log::Level::Trace) {
            trace!(
                "get account summary {} for {} at maximum topoheight {}",
                asset,
                key.as_address(self.is_mainnet()),
                max_topoheight
            );
            ...
            .await?
        {
            if topo < min_topoheight {
                if log::log_enabled!(log::Level::Trace) {
                    trace!(
                        "No changes found for {} above min topoheight {}",
                        key.as_address(self.is_mainnet()),
                        min_topoheight
                    );
                }
            ...
            if version.contains_output() {
                if log::log_enabled!(log::Level::Trace) {
                    trace!(
                        "Stable with output balance found for {} at topoheight {}",
                        key.as_address(self.is_mainnet()),
                        topo
                    );
                }
    ...
    async fn get_spendable_balances_for(
        ...
        if log::log_enabled!(log::Level::Trace) {
            trace!(
                "get spendable balances for {} at maximum topoheight {}",
                key.as_address(self.is_mainnet()),
                max_topoheight
            );
        }
        ...
        while let Some(topo) = fetch_topoheight
            .take()
