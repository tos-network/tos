...
use tos_common::{
    block::TopoHeight, crypto::PublicKey, transaction::MultiSigPayload, versioned_type::Versioned,
};
...
pub trait MultiSigProvider {
    ...
    async fn get_last_multisig<'a>(
        ...
        let topoheight = self
            .get_last_topoheight_for_multisig(account)
            .await?
            .ok_or(BlockchainError::NoMultisig)?;

        let state = self
            .get_multisig_at_topoheight_for(account, topoheight)
            .await?;
