...
macro_rules! init_cache {
    ($cache_size: expr) => {{
        Mutex::new(LruCache::new(crate::core::storage::cache::non_zero_usize(
            $cache_size,
        )))
    }};
}
...
fn non_zero_usize(value: usize) -> NonZeroUsize {
    NonZeroUsize::new(value).unwrap_or_else(|| {
        // SAFETY: fallback to 1 when provided value is 0.
...
impl Default for ChainCache {
    fn default() -> Self {
        Self {
            tip_base_cache: Mutex::new(LruCache::new(non_zero_usize(DEFAULT_CACHE_SIZE))),
            tip_work_score_cache: Mutex::new(LruCache::new(non_zero_usize(DEFAULT_CACHE_SIZE))),
            common_base_cache: Mutex::new(LruCache::new(non_zero_usize(DEFAULT_CACHE_SIZE))),
            full_order_cache: Mutex::new(LruCache::new(non_zero_usize(DEFAULT_CACHE_SIZE))),
            height: 0,
            topoheight: 0,
            stable_height: 0,
            stable_topoheight: 0,
