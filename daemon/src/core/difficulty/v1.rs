...
use tos_common::{
    difficulty::Difficulty, time::TimestampMillis, utils::format_difficulty, varuint::VarUint,
};
...
pub fn calculate_difficulty(
    ...
    target_block_time: TimestampMillis,
) -> (Difficulty, VarUint) {
    let z = previous_difficulty / solve_time;
    trace!(
        "Calculating difficulty v1, solve time: {}, previous_difficulty: {}, z: {}, p: {}",
    ...
    let (x_est_new, p_new) = kalman_filter(
        z,
        previous_difficulty / target_block_time,
        p,
        SHIFT,
        LEFT_SHIFT,
        PROCESS_NOISE_COVAR,
...
mod tests {
    ...
    fn test_kalman_filter_v1() {
        ...
        let (x_est_new, p_new) =
            kalman_filter(z, VarUint::one(), P, SHIFT, LEFT_SHIFT, PROCESS_NOISE_COVAR);
        assert_eq!(x_est_new, VarUint::one());
        assert_eq!(p_new, VarUint::from_u64(4501837440));

        let (x_est_new, p_new) = kalman_filter(
            DEFAULT_DIFFICULTY / VarUint::from_u64(2000),
            x_est_new,
            p_new,
            SHIFT,
            LEFT_SHIFT,
