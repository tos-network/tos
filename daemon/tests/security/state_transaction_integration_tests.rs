...
use crate::test_utilities::{
    MockTransaction, MockAccount, MockStorage, MockMempool,
    AtomicNonceChecker, test_hash
};
...
async fn test_v01_multi_transaction_sequence_lifecycle() {
    ...
    for tx in &transactions {
        ...
        assert!(sender_balance >= tx.amount,
        ...
        storage.set_balance(&tx.receiver, new_receiver_balance).await;

        // Step 5: Update nonce
        nonce_checker.compare_and_swap(&tx.sender, current_nonce, current_nonce + 1).await
    ...
    assert_eq!(storage.get_nonce(alice).await.unwrap(), 5,
        "Alice should have nonce 5 after 5 transactions");
    assert_eq!(storage.get_nonce(bob).await.unwrap(), 4,
        "Bob should have nonce 4 after 4 transactions");
    assert_eq!(storage.get_nonce(charlie).await.unwrap(), 3,
    ...
    assert_eq!(storage.get_balance(alice).await.unwrap(), 9500,
        "Alice final balance should be 9500");
    assert_eq!(storage.get_balance(bob).await.unwrap(), 5195,
        "Bob final balance should be 5195");
    assert_eq!(storage.get_balance(charlie).await.unwrap(), 3305,
        "Charlie final balance should be 3305");

    if log::log_enabled!(log::Level::Info) {
        log::info!("test_v01: Successfully processed 12 sequential transactions with balance conservation");
    }
}
...
async fn test_v02_cross_account_transaction_chains() {
    ...
    state.transfer("alice", "bob", 500).await
        .expect("Alice → Bob transfer should succeed");
    state.transfer("bob", "charlie", 300).await
        .expect("Bob → Charlie transfer should succeed");
    state.transfer("charlie", "dave", 200).await
        .expect("Charlie → Dave transfer should succeed");
    state.transfer("dave", "eve", 100).await
        .expect("Dave → Eve transfer should succeed");

    // Verify balances after chain
    assert_eq!(state.get_balance("alice").await.unwrap(), 500,
        "Alice: 1000 - 500 = 500");
    assert_eq!(state.get_balance("bob").await.unwrap(), 1200,
        "Bob: 1000 + 500 - 300 = 1200");
    assert_eq!(state.get_balance("charlie").await.unwrap(), 1100,
        "Charlie: 1000 + 300 - 200 = 1100");
    assert_eq!(state.get_balance("dave").await.unwrap(), 1100,
        "Dave: 1000 + 200 - 100 = 1100");
    assert_eq!(state.get_balance("eve").await.unwrap(), 1100,
        "Eve: 1000 + 100 = 1100");

    // Verify total supply conservation
    let final_supply: u64 = accounts.iter()
        .map(|a| state.get_balance(a).await.unwrap())
        .sum();
    assert_eq!(initial_supply, final_supply,
    ...
    state.transfer("eve", "dave", 50).await
        .expect("Eve → Dave transfer should succeed");
    state.transfer("dave", "charlie", 75).await
        .expect("Dave → Charlie transfer should succeed");
    state.transfer("charlie", "bob", 100).await
        .expect("Charlie → Bob transfer should succeed");
    state.transfer("bob", "alice", 125).await
        .expect("Bob → Alice transfer should succeed");

    // Verify final balances
    assert_eq!(state.get_balance("alice").await.unwrap(), 625,
        "Alice: 500 + 125 = 625");
    assert_eq!(state.get_balance("bob").await.unwrap(), 1175,
        "Bob: 1200 + 100 - 125 = 1175");
    assert_eq!(state.get_balance("charlie").await.unwrap(), 1025,
        "Charlie: 1100 + 75 - 100 = 1025");
    assert_eq!(state.get_balance("dave").await.unwrap(), 1075,
        "Dave: 1100 + 50 - 75 = 1075");
    assert_eq!(state.get_balance("eve").await.unwrap(), 1050,
        "Eve: 1100 - 50 = 1050");
...
async fn test_v03_state_queries_during_processing() {
    ...
    let transfer_handle = tokio::spawn(async move {
        state_tx.transfer("alice", "bob", 500).await
    });
    ...
    for (alice, bob) in results.iter() {
        ...
        assert!(
            ...
            "Query must see consistent snapshot: ({}, {})", alice, bob
        );
    }

    if log::log_enabled!(log::Level::Info) {
        log::info!("test_v03: Verified {} queries all saw consistent snapshots", results.len());
    }
}
...
async fn test_v04_nonce_management_concurrent_submissions() {
    ...
    impl NonceManager {
        ...
        async fn submit_transaction(&self, tx_nonce: u64) -> Result<(), String> {
            ...
            if tx_nonce != expected_nonce {
                return Err(format!("Invalid nonce: expected {}, got {}", expected_nonce, tx_nonce));
            }
    ...
    assert_eq!(success_count, NUM_TRANSACTIONS,
        "All sequential nonce submissions should succeed");

    // Verify final nonce
    assert_eq!(manager.get_current_nonce(), NUM_TRANSACTIONS as u64,
        "Final nonce should be {}", NUM_TRANSACTIONS);

    // Verify all nonces were used
    let used = manager.get_used_nonces().await;
    assert_eq!(used.len(), NUM_TRANSACTIONS,
        "All {} nonces should be marked as used", NUM_TRANSACTIONS);

    // Verify nonce sequence is complete (0..19)
    for nonce in 0..NUM_TRANSACTIONS {
        assert!(used.contains(&(nonce as u64)),
            "Nonce {} should be in used set", nonce);
    }
    ...
    if log::log_enabled!(log::Level::Info) {
        log::info!("test_v04: Successfully validated {} concurrent nonce submissions with monotonicity", NUM_TRANSACTIONS);
    }
}
...
async fn test_v05_failed_transaction_rollback() {
    ...
    impl TransactionExecutor {
        ...
        async fn execute_transaction(
            ...
            // Begin transaction
            self.execution_log.lock().await.push(format!("BEGIN: {} → {} ({})", from, to, amount));
            ...
            // Update balances
            {
                ...
                if from_balance < amount {
                    self.execution_log.lock().await.push(format!("FAIL: Insufficient balance"));
                    return Err("Insufficient balance".to_string());
                }
            ...
            if should_fail {
                ...
                *self.nonces.write().await = nonce_snapshot;
                self.execution_log.lock().await.push(format!("ROLLBACK: State restored"));
                return Err("Transaction execution failed".to_string());
            }

            // Success
            self.execution_log.lock().await.push(format!("COMMIT: Transaction succeeded"));
            Ok(())
        }
    ...
    executor.execute_transaction("alice", "bob", 100, false).await
        .expect("Transaction should succeed");

    assert_eq!(executor.get_balance("alice").await, 900,
        "Alice balance should be updated after successful transaction");
    assert_eq!(executor.get_balance("bob").await, 600,
        "Bob balance should be updated after successful transaction");
    assert_eq!(executor.get_nonce("alice").await, 1,
        "Alice nonce should be incremented after successful transaction");

    // Execute failing transaction (should rollback)
    let fail_result = executor.execute_transaction("alice", "bob", 100, true).await;
    ...
    assert_eq!(total_balance, initial_alice_balance + initial_bob_balance,
...
async fn test_v06_account_state_transitions() {
    ...
    impl AccountStateManager {
        ...
        async fn create_account(&self, address: String, initial_balance: u64) -> Result<(), String> {
            ...
            self.creation_log.lock().await.push(format!("CREATED: {} with balance {}", address, initial_balance));
            Ok(())
        }
        ...
        async fn update_balance(&self, address: &str, delta: i64) -> Result<(), String> {
            ...
            self.creation_log.lock().await.push(format!("UPDATED: {} balance by {}", address, delta));
            Ok(())
        }
        ...
        async fn increment_nonce(&self, address: &str) -> Result<(), String> {
            ...
            self.creation_log.lock().await.push(format!("NONCE_INC: {} → {}", address, account.nonce));
            Ok(())
        }

        async fn delete_account(&self, address: &str) -> Result<MockAccount, String> {
            let mut accounts = self.accounts.write().await;
            let account = accounts.remove(address)
                .ok_or_else(|| format!("Account {} not found", address))?;

            self.creation_log.lock().await.push(format!("DELETED: {}", address));
            Ok(account)
        }
    ...
    manager.create_account("alice".to_string(), 1000).await
        .expect("Account creation should succeed");

    assert!(manager.account_exists("alice").await,
    ...
    manager.update_balance("alice", 500).await
        .expect("Balance increase should succeed");
    let alice = manager.get_account("alice").await.unwrap();
    assert_eq!(alice.balance, 1500);

    manager.update_balance("alice", -300).await
    ...
    for _ in 0..5 {
        manager.increment_nonce("alice").await
    ...
    manager.create_account("bob".to_string(), 500).await
        .expect("Bob account creation should succeed");
    manager.create_account("charlie".to_string(), 750).await
    ...
    let deleted_alice = manager.delete_account("alice").await
...
async fn test_v07_balance_conservation_during_reorg() {
    ...
    impl ReorgChain {
        ...
        async fn apply_block(&self, transactions: Vec<(String, String, u64)>) -> Result<(), String> {
            let mut balances = self.balances.write().await;
        ...
        async fn rollback_blocks(&self, num_blocks: usize, initial_balances: HashMap<String, u64>) -> Result<(), String> {
    ...
    chain.rollback_blocks(5, initial_balances.clone()).await
    ...
    assert_ne!(alt_alice, main_alice,
