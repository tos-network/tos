...
use tos_common::{
    ...
    rpc::{Id, InternalRpcError, RPCHandler, RpcResponse, RpcResponseError},
    tokio::sync::RwLock,
};

use crate::api::{ApplicationDataRelayer, XSWDError};

use super::{AppState, AppStateShared, OnRequestResult, XSWDHandler, XSWDProvider, XSWD};
    ...
    W: Clone + Send + Sync + XSWDHandler + 'static,
{
    ...
    pub async fn close(&self) {
        ...
        stream::iter(applications.drain())
            .for_each_concurrent(self.concurrency, |(_, (client, _))| async move {
                client.close().await
            })
            .await;
    }
    ...
    pub async fn add_application(
        self: &XSWDRelayerShared<W>,
        app_data: ApplicationDataRelayer,
    ) -> Result<(), anyhow::Error> {
        // Sanity check
        self.xswd
            .verify_application(self.as_ref(), &app_data.inner)
            .await?;
    ...
    pub async fn on_message(
        ...
        match self.xswd.on_request(self, state, message).await? {
            ...
            OnRequestResult::Request {
                ...
                let mut applications = self.applications.write().await;
                let (_, events) = applications.get_mut(state).ok_or_else(|| {
                    RpcResponseError::new(request.id.clone(), XSWDError::ApplicationNotFound)
                })?;
