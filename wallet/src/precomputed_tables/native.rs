use std::{fs::create_dir_all, path::Path, sync::Arc, time::Instant};

use anyhow::{bail, Result};
use log::info;
use tos_common::{crypto::ecdlp, utils::detect_available_parallelism};
...
pub async fn read_or_generate_precomputed_tables<
    P: ecdlp::ProgressTableGenerationReportFunction + Send + Sync,
>(
    path: Option<&str>,
    l1: usize,
    progress_report: P,
    ...
    let tables = if Path::new(&full_path).exists() {
        ...
        let tables = ecdlp::ECDLPTables::generate_with_progress_report_par(
            l1,
        ...
        if log::log_enabled!(log::Level::Info) {
            info!(
                "Took {:?} to generate the precomputed tables",
                instant.elapsed()
            );
        }
