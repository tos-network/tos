...
CREATE AWS INSTANCES:
	1. create a client instance, called `tos-client`
	2. create as many authority instances as required, called `tos-authorituy-#` (where # is a number)
	3. create a security group called `tos` and add all authorities to it
	4. open the following ports on the tos security group
		TCP 22
		UDP ALL
...
def set_hosts(value ='running', instance_type='tos', region=region):
...
USE THE TESTNET:
	1. locate the file 'committee.json' specifying the set of Tos authorities
...
def initialize_client(remote_client=remote_client):
	if remote_client:
		...
		local('(cd ../tos && cargo build --release)')
