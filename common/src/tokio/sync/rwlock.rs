//! Debug-mode RwLock wrapper with deadlock detection diagnostics
//!
//! **Note**: This module is only compiled when the `deadlock-detection` feature is enabled.
//! In production builds, the parent module re-exports `tokio::sync::RwLock` directly.
//!
...
//! 2. **Unreachable in practice**: Tokio's async RwLock does not poison (unlike `std::sync::RwLock`)
