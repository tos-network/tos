...
pub fn build_environment<P: ContractProvider>() -> EnvironmentBuilder<'static> {
    ...
    // Balance simplification: Ciphertext functions removed (not needed for plaintext balances)
    // The entire Ciphertext block (add, sub, mul, div, new) has been removed

    // Misc
    {
        // Balance simplification: get_account_balance_of returns (u64, u64) instead of (u64, Ciphertext)
        env.register_native_function(
...
// Balance simplification: Returns (topoheight, balance) as (u64, u64) instead of (u64, Ciphertext)
