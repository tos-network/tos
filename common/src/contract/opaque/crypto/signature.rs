...
impl_opaque!("Signature", Signature);
impl_opaque!("Signature", Signature, json);
...
pub fn signature_from_bytes_fn(
    _: FnInstance,
    mut params: FnParams,
    _: &mut Context,
) -> FnReturnType {
    ...
    let bytes_array: [u8; SIGNATURE_SIZE] = bytes
        .try_into()
        .map_err(|_| EnvironmentError::InvalidParameter)?;

    let signature = Signature::from_bytes(&bytes_array).context("signature from bytes")?;
    Ok(Some(Primitive::Opaque(signature.into()).into()))
}

pub fn signature_verify_fn(
    zelf: FnInstance,
    mut params: FnParams,
    _: &mut Context,
) -> FnReturnType {
    ...
    Ok(Some(
        Primitive::Boolean(signature.verify(&data, &key)).into(),
    ))
}
