...
pub fn storage(_: FnInstance, _: FnParams, _: &mut Context) -> FnReturnType {
    Ok(Some(
        Primitive::Opaque(OpaqueWrapper::new(OpaqueStorage)).into(),
    ))
}

pub fn storage_load<P: ContractProvider>(
    _: FnInstance,
    mut params: FnParams,
    context: &mut Context,
) -> FnReturnType {
        ...
        None => match storage.load_data(&state.contract, &key, state.topoheight)? {
            Some((topoheight, constant)) => {
                state.cache.storage.insert(
                    key.clone(),
                    (VersionedState::FetchedAt(topoheight), constant.clone()),
                );
                constant
            }
            None => None,
...
pub fn storage_has<P: ContractProvider>(
    _: FnInstance,
    mut params: FnParams,
    context: &mut Context,
) -> FnReturnType {
...
pub fn storage_store<P: ContractProvider>(
    _: FnInstance,
    mut params: FnParams,
    context: &mut Context,
) -> FnReturnType {
...
pub fn storage_delete<P: ContractProvider>(
    _: FnInstance,
    mut params: FnParams,
    context: &mut Context,
) -> FnReturnType {
