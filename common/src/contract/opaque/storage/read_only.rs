...
pub fn read_only_storage<P: ContractProvider>(
    _: FnInstance,
    mut parameters: FnParams,
    context: &mut Context,
) -> FnReturnType {
    let (storage, state) = from_context::<P>(context)?;
    let hash: Hash = parameters.remove(0).into_owned()?.into_opaque_type()?;
    ...
    Ok(Some(
        Primitive::Opaque(OpaqueWrapper::new(OpaqueReadOnlyStorage(hash))).into(),
    ))
}

pub fn read_only_storage_load<P: ContractProvider>(
    zelf: FnInstance,
    mut params: FnParams,
    context: &mut Context,
) -> FnReturnType {
    let (storage, state) = from_context::<P>(context)?;
    let zelf: &OpaqueReadOnlyStorage = zelf?.as_opaque_type()?;
...
pub fn read_only_storage_has<P: ContractProvider>(
    zelf: FnInstance,
    mut params: FnParams,
    context: &mut Context,
) -> FnReturnType {
