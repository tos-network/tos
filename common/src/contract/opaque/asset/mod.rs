...
use crate::{
    contract::{
        from_context, get_asset_changes_for_hash, get_asset_changes_for_hash_mut,
        get_balance_from_cache, ChainState, ContractOutput, ContractProvider,
    },
    crypto::Hash,
...
pub fn asset_get_max_supply<P: ContractProvider>(
    zelf: FnInstance,
    _: FnParams,
    context: &mut Context,
) -> FnReturnType {
...
pub fn asset_get_contract_hash<P: ContractProvider>(
    zelf: FnInstance,
    _: FnParams,
    context: &mut Context,
) -> FnReturnType {
...
// Contract hash that created this asset
pub fn asset_get_contract_id<P: ContractProvider>(
    zelf: FnInstance,
    _: FnParams,
    context: &mut Context,
) -> FnReturnType {
...
pub fn asset_get_supply<P: ContractProvider>(
    zelf: FnInstance,
    _: FnParams,
    context: &mut Context,
) -> FnReturnType {
    ...
    let supply = provider
        .load_asset_supply(&asset.hash, topoheight)?
...
pub fn asset_get_name(zelf: FnInstance, _: FnParams, context: &mut Context) -> FnReturnType {
    ...
    let changes = get_asset_changes_for_hash(state, &asset.hash)?;
    Ok(Some(
        Primitive::String(changes.data.1.get_name().to_owned()).into(),
    ))
}
...
pub fn asset_get_ticker(zelf: FnInstance, _: FnParams, context: &mut Context) -> FnReturnType {
    ...
    let changes = get_asset_changes_for_hash(state, &asset.hash)?;
    Ok(Some(
        Primitive::String(changes.data.1.get_ticker().to_owned()).into(),
    ))
}
...
pub fn asset_is_read_only(zelf: FnInstance, _: FnParams, context: &mut Context) -> FnReturnType {
    ...
    let changes = get_asset_changes_for_hash(state, &asset.hash)?;
    let read_only = changes
        .data
        .1
        .get_owner()
        .as_ref()
        .map(|v| v.get_contract())
        != Some(state.contract);

    Ok(Some(Primitive::Boolean(read_only).into()))
}

pub fn asset_transfer_ownership<P: ContractProvider>(
    zelf: FnInstance,
    mut params: FnParams,
    context: &mut Context,
) -> FnReturnType {
    let param: Hash = params.remove(0).into_owned()?.into_opaque_type()?;
    ...
    let changes = get_asset_changes_for_hash_mut(state, &asset.hash)?;
    Ok(Some(
        match changes.data.1.get_owner_mut() {
...
pub fn asset_mint<P: ContractProvider>(
    zelf: FnInstance,
    params: FnParams,
    context: &mut Context,
) -> FnReturnType {
    ...
    let read_only = asset_data.get_owner().as_ref().map(|v| v.get_contract()) != Some(&contract);
    ...
    match get_balance_from_cache(provider, chain_state, asset.hash.clone())? {
        ...
        v => *v = Some((VersionedState::New, amount)),
    };
