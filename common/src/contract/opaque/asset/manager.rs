...
use crate::{
    ...
    contract::{
        from_context, get_optional_asset_from_cache, AssetChanges, ContractOutput, ContractProvider,
    },
...
fn is_valid_str_for_asset(name: &str, whitespace: bool, uppercase_only: bool) -> bool {
    ...
    name.chars()
        .all(|c| is_valid_char_for_asset(c, whitespace, uppercase_only))
}
...
pub fn asset_create<P: ContractProvider>(
    _: FnInstance,
    mut params: FnParams,
    context: &mut Context,
) -> FnReturnType {
    ...
    let decimals = params.remove(3).into_owned()?.to_u8()?;

    let ticker = params.remove(2).into_owned()?.into_string()?;

    if ticker.len() > TICKER_LEN {
        return Err(EnvironmentError::Expect("Asset ticker is too long".to_owned()));
    }

    // Ticker can be ASCII & upper case only
    // No whitespace is allowed in it
    if !is_valid_str_for_asset(&ticker, false, true) {
        return Err(EnvironmentError::Expect("Asset ticker must be ASCII only".to_owned()));
    }

    let name = params.remove(1).into_owned()?.into_string()?;
    if name.len() > u8::MAX as usize {
        return Err(EnvironmentError::Expect("Asset name is too long".to_owned()));
    }

    // Name can be ASCII only
    if !is_valid_str_for_asset(&name, true, false) {
        return Err(EnvironmentError::Expect("Asset name must be ASCII only".to_owned()));
    }
    ...
    let data = AssetData::new(
        ...
        ticker,
        max_supply,
        Some(AssetOwner::new(chain_state.contract.clone(), id)),
    );
    chain_state.assets.insert(
        asset_hash.clone(),
        Some(AssetChanges {
    ...
    chain_state.outputs.push(ContractOutput::NewAsset {
        asset: asset_hash.clone(),
    });

    // Pay the cost for a new token
    context.increase_gas_usage(COST_PER_TOKEN)?;

    let asset = Asset { hash: asset_hash };
    Ok(Some(Primitive::Opaque(asset.into()).into()))
}

pub fn asset_get_by_id<P: ContractProvider>(
    _: FnInstance,
    params: FnParams,
    context: &mut Context,
) -> FnReturnType {
...
pub fn asset_get_by_hash<P: ContractProvider>(
    _: FnInstance,
    mut params: FnParams,
    context: &mut Context,
) -> FnReturnType {
    let hash: Hash = params.remove(0).into_owned()?.into_opaque_type()?;
...
mod tests {
    ...
    fn test_various_asset_names() {
        ...
        assert!(is_valid_str_for_asset("TOS99", true, false));
