...
pub use vrf::{compute_vrf_input, BlockVrfData};
...
pub fn get_combined_hash_for_tips<H: AsRef<Hash>, I: Iterator<Item = H>>(tips: I) -> Hash {
    let mut bytes = [0u8; HASH_SIZE];
    for tip in tips {
        for (i, byte) in tip.as_ref().as_bytes().iter().enumerate() {
            bytes[i] ^= byte;
        }
    }
    Hash::new(bytes)
