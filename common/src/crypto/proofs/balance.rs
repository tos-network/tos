use crate::{
    crypto::{
        elgamal::{Ciphertext, PedersenCommitment, PedersenOpening, PublicKey},
        proofs::{BatchCollector, CommitmentEqProof, ProofVerificationError},
...
impl BalanceProof {
    ...
    pub fn new(keypair: &KeyPair, amount: u64, ciphertext: Ciphertext) -> Self {
        Self::prove(
            ...
            amount,
            ciphertext,
            &mut Transcript::new(b"balance_proof"),
        )
    }
    ...
    pub fn prove(
        ...
        let commitment_eq_proof =
            CommitmentEqProof::new(keypair, &zeroed_balance, &Self::OPENING, 0, transcript);
    ...
    pub fn pre_verify(
        ...
        let destination_commitment =
            PedersenCommitment::new_with_opening(Scalar::ZERO, &Self::OPENING);
