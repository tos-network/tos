use super::{
    BatchCollector, CommitmentEqProof, ProofGenerationError, ProofVerificationError, BP_GENS,
    BULLET_PROOF_SIZE, PC_GENS,
};
use crate::{
    crypto::{
        elgamal::{
            Ciphertext, CompressedCommitment, PedersenCommitment, PedersenOpening, PublicKey,
        },
...
impl OwnershipProof {
    ...
    pub fn prove(
        ...
        let left_commitment = PedersenCommitment::new_with_opening(left, &opening).compress();
        ...
        let ct = keypair
            .get_public_key()
            .encrypt_with_opening(amount, &Self::OPENING);
        let ct_left = ciphertext - ct;

        // Generate the proof that the final balance is ? minus N after applying the commitment.
        let commitment_eq_proof =
            CommitmentEqProof::new(keypair, &ct_left, &opening, left, transcript);

        // Create a range proof to prove that whats left is >= 0
        let (range_proof, range_commitment) = RangeProof::prove_single(
            &BP_GENS,
            &PC_GENS,
            transcript,
            left,
    ...
    pub fn pre_verify(
        ...
        self.range_proof.verify_single(
            ...
            transcript,
            &(
                commitment.as_point().clone(),
                self.commitment.as_point().clone(),
            ),
            BULLET_PROOF_SIZE,
        )?;
...
mod tests {
    ...
    fn test_fake_commitment_ownership_proof() {
        ...
        let ct = keypair
            .get_public_key()
            .encrypt_with_opening(amount + inflate, &OwnershipProof::OPENING);
        ...
        let commitment_eq_proof = CommitmentEqProof::new_with_scalar(
            &keypair,
            &ct_left,
            &opening,
            left_scalar,
