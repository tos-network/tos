...
pub async fn create_test_rocksdb_storage() -> Arc<RwLock<RocksStorage>> {
    ...
    let config = RocksDBConfig {
        parallelism,
        max_background_jobs: parallelism,
        max_subcompaction_jobs: parallelism,
        low_priority_background_threads: parallelism,
        max_open_files: 1024,
        keep_max_log_files: 4,
        compression_mode: CompressionMode::default(),
        cache_mode: CacheMode::default(),
        cache_size: 1024 * 1024, // 1MB cache for tests
        write_buffer_size: 64 * 1024 * 1024,
        write_buffer_shared: false,
    };

    let mut storage = RocksStorage::new(&dir_path, Network::Devnet, &config);
