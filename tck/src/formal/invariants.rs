...
pub fn check_balance_conservation(
    ...
    let actual_total = balances
        .iter()
        .try_fold(0u64, |acc, &balance| acc.checked_add(balance))
        .ok_or(InvariantViolation::BalanceSumOverflow)?;
...
pub enum InvariantViolation {
    ...
    /// Balance sum overflowed during calculation
    #[error("Balance sum overflow: total balance exceeds u64::MAX")]
    BalanceSumOverflow,
...
mod tests {
    ...
    #[test]
    fn test_balance_conservation_overflow() {
        // This should detect overflow instead of wrapping
        let balances = vec![u64::MAX, 1];
        let result = check_balance_conservation(&balances, 0);
        assert!(matches!(
            result,
            Err(InvariantViolation::BalanceSumOverflow)
        ));
    }
