...
fn test_validate_tips_91pct_boundary() {
    ...
    assert!(
        !validate_tip_91pct(1000, 500),
        "diff=500 must be rejected"
    );
...
fn test_combined_filter_and_clamp_mirrors_c_clamp_test() {
    struct Tip { id: u64, difficulty: u64, cumul: u64 }
    let tips = vec![
        Tip { id: 1, difficulty: 1000, cumul: 5000 },  // best
        Tip { id: 2, difficulty:  992, cumul: 4000 },
        Tip { id: 3, difficulty:  991, cumul: 3000 },
        Tip { id: 4, difficulty:  950, cumul: 2000 },
        Tip { id: 5, difficulty:  920, cumul: 1000 },
    ];
    ...
    assert!(validate_tip_91pct(best_diff, 920), "920 > threshold=910, passes 91% filter");
    assert!(validate_tip_91pct(best_diff, 950), "950 > threshold=910, passes 91% filter");
}
